...
const expectedCommit='65ec059bde30d98c92165b24a30a540800dd1f6f';
if(execFileSync('git',['-C',root,'rev-parse','HEAD'],{encoding:'utf8'}).trim()!==expectedCommit)throw Error('Upstream version differs: review upgrade before applying this localization.');
...
const report=[];
async function restoreNoLongerTranslated(dir){for(const entry of await fs.readdir(dir,{withFileTypes:true})){const p=path.join(dir,entry.name);if(entry.isDirectory())await restoreNoLongerTranslated(p);else{const relative=path.relative(path.join(kit,'integration/source-before'),p).replaceAll('\\','/');if(!groups.has(relative))await fs.copyFile(p,path.join(root,relative));}}}
try{await restoreNoLongerTranslated(path.join(kit,'integration/source-before'));}catch(e){if(e.code!=='ENOENT')throw e;}
for(const [relative,edits] of groups){
 const file=path.join(root,relative);
 const backup=path.join(kit,'integration/source-before',relative);
 let original;
