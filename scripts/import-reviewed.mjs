...
for(const r of reviewed){
 let sf=files.get(r.sourceFile);if(!sf){let source;try{source=await fs.readFile(path.join(kit,'integration/source-before',r.sourceFile),'utf8');}catch{source=await fs.readFile(path.join(root,r.sourceFile),'utf8');}sf=ts.createSourceFile(r.sourceFile,source,ts.ScriptTarget.Latest,true,r.sourceFile.endsWith('tsx')?ts.ScriptKind.TSX:ts.ScriptKind.TS);files.set(r.sourceFile,sf);}
 const matches=[];
