...
const ACTION_DOMAINS: { value: string; label: string }[] = [
  ...
  { value: "tool_gateway.", label: l10n("local.tools_ea93d6a2") },
    ...
    <div className="space-y-4">
      {!hideHeader ? (
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-lg font-semibold text-foreground">{l10n("local.activity_38da1505")}</h1>
            <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
              {resolvedMode === "agents"
                ? l10n("local.every_recorded_agent_action_newest_first_with_693aedcd")
                : l10n("local.everything_happening_in_your_company_newest_f_455266f7")}
            </p>
          </div>
        </div>
      ) : null}
      ...
      {showModeToggle ? (
        <Tabs value={resolvedMode} onValueChange={(value) => onModeChange?.(value as AuditFeedMode)}>
          <TabsList aria-label={l10n("local.activity_scope_98a37b44")}>
            <TabsTrigger value="all">{l10n("local.all_activity_29ebb2ef")}</TabsTrigger>
            <TabsTrigger value="agents">{l10n("local.agent_actions_a21eef04")}</TabsTrigger>
          </TabsList>
        </Tabs>
      ) : null}
              ...
              <p className="mt-1 max-w-md text-sm text-muted-foreground">
                {hasActiveFilters
                  ...
                  : resolvedMode === "agents"
                    ...
                    : l10n("local.as_soon_as_anyone_in_your_company_does_someth_29840ec0")}
              </p>
            </div>
            {hasActiveFilters ? (
