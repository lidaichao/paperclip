  ...
  if (!resolvedCompanyId) {
    ...
    return (
      <div className="space-y-4">
        <p className="text-sm text-muted-foreground">{l10n("local.select_an_organization_to_view_this_page_5a2b723e")}</p>
      </div>
    );
  }

  if (!contributions) {
    return <div className="text-sm text-muted-foreground">{l10n("local.loading_ba3bbbe1")}</div>;
  }
    ...
    if (duplicateMatches.length > 1) {
      return (
        <div className="rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm text-destructive">
          {l10n("local.multiple_plugins_declare_the_route_9f13099a")}{" "}<code>{pluginRoutePath}</code>{l10n("local._use_the_plugin_id_route_until_the_conflict_i_d633310e")}</div>
      );
    }
  }
