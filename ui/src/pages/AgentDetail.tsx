...
const sourceLabels: Record<string, string> = {
  timer: l10n("local.timer_cdeec2b3"),
  assignment: l10n("local.assignment_153cdaae"),
  on_demand: l10n("local.on_demand_ceaa2444"),
...
const LEGACY_AGENT_DETAIL_TABS = [
  { value: "dashboard", label: l10n("local.dashboard_67b69646") },
  ...
  { value: "runs", label: l10n("local.runs_848f54e8") },
  { value: "audit", label: l10n("local.audit_bb6aea28") },
  { value: "budget", label: l10n("local.budget_1c6225ec") },
] as const;

export const DISCARD_AGENT_CONFIG_CHANGES_MESSAGE = l10n("local.discard_unsaved_agent_configuration_changes_316cb031");
...
export function RunInvocationCard({
  ...
  return (
    <div className="rounded-lg border border-border bg-background/60 p-3 space-y-2">
      <div className="text-xs font-medium text-muted-foreground">{l10n("local.invocation_05adee5d")}</div>
      {typeof payload.adapterType === "string" && (
        <div className="text-xs"><span className="text-muted-foreground">{l10n("local.adapter_6fada5c7")}{" "}</span>{payload.adapterType}</div>
      )}
      {typeof payload.cwd === "string" && (
        <div className="text-xs break-all"><span className="text-muted-foreground">{l10n("local.working_dir_802469e9")}{" "}</span><span className="font-mono">{payload.cwd}</span></div>
      )}
      {hasAdvancedDetails && (
        <Collapsible>
          <CollapsibleTrigger className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground transition-colors group">
            <ChevronRight className="h-3 w-3 transition-transform group-data-[state=open]:rotate-90" />
            {l10n("local.details_45989de4")}</CollapsibleTrigger>
          <CollapsibleContent className="pt-2 space-y-2">
            {commandLine && (
              <div className="text-xs break-all">
                <span className="text-muted-foreground">{l10n("local.command_a7a9c915")}{" "}</span>
                <span className="font-mono">{commandLine}</span>
              </div>
            )}
            {Array.isArray(payload.commandNotes) && payload.commandNotes.length > 0 && (
              <div>
                <div className="text-xs text-muted-foreground mb-1">{l10n("local.command_notes_9d924bb2")}</div>
            ...
            {payload.prompt !== undefined && (
              <div>
                <div className="text-xs text-muted-foreground mb-1">{l10n("local.prompt_5c391238")}</div>
            ...
            {payload.context !== undefined && (
              <div>
                <div className="text-xs text-muted-foreground mb-1">{l10n("local.context_a6e600a1")}</div>
                <pre className="bg-neutral-100 dark:bg-neutral-950 rounded-md p-2 text-xs overflow-x-auto whitespace-pre-wrap">
            ...
            {payload.env !== undefined && (
              <div>
                <div className="text-xs text-muted-foreground mb-1">{l10n("local.environment_9e471951")}</div>
...
function WorkspaceOperationLogViewer({
  ...
  return (
    <div className="space-y-2">
      ...
      >
        {open ? l10n("local.hide_full_log_747af741") : l10n("local.show_full_log_4a55ed01")}
      </button>
      {open && (
        <div className="rounded-md border border-border bg-background/70 p-2">
          {isLoading && <div className="text-xs text-muted-foreground">{l10n("local.loading_log_e838607e")}</div>}
          {error && (
            <div className="text-xs text-destructive">
              {error instanceof Error ? error.message : l10n("local.failed_to_load_workspace_operation_log_42a6e0b2")}
            </div>
          )}
          {!isLoading && !error && chunks.length === 0 && (
            <div className="text-xs text-muted-foreground">{l10n("local.no_persisted_log_lines_9b93ec6b")}</div>
          )}
          {chunks.length > 0 && (
  ...
  return (
    <div className="rounded-lg border border-border bg-background/60 p-3 space-y-3">
      <div className="text-xs font-medium text-muted-foreground">
        {l10n("local.workspace_2452b15a")}{operations.length})
      </div>
            ...
            <div key={operation.id} className="rounded-md border border-border/70 bg-background/70 p-3 space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="text-(length:--text-micro) text-muted-foreground">
                  ...
                  {operation.finishedAt && (" " + l10n("local.to_value_bc283e29", {v0: (relativeTime(operation.finishedAt))}))}
                </div>
              </div>
              {operation.command && (
                <div className="text-xs break-all">
              ...
              {operation.cwd && (
                <div className="text-xs break-all">
                  <span className="text-muted-foreground">{l10n("local.working_dir_802469e9")}{" "}</span>
              ...
              {(asNonEmptyString(metadata?.branchName)
                ...
                <div className="grid gap-1 text-xs sm:grid-cols-2">
                  {asNonEmptyString(metadata?.branchName) && (
                    <div><span className="text-muted-foreground">{l10n("local.branch_b8a50d7f")}{" "}</span><span className="font-mono">{metadata?.branchName as string}</span></div>
                  )}
                  {asNonEmptyString(metadata?.baseRef) && (
                    <div><span className="text-muted-foreground">{l10n("local.base_ref_f42675a4")}{" "}</span><span className="font-mono">{metadata?.baseRef as string}</span></div>
                  )}
                  {asNonEmptyString(metadata?.worktreePath) && (
                    <div className="break-all"><span className="text-muted-foreground">{l10n("local.worktree_b7f496bf")}{" "}</span><span className="font-mono">{metadata?.worktreePath as string}</span></div>
                  )}
                  {asNonEmptyString(metadata?.repoRoot) && (
                    <div className="break-all"><span className="text-muted-foreground">{l10n("local.repo_root_72decaf8")}{" "}</span><span className="font-mono">{metadata?.repoRoot as string}</span></div>
                  )}
                  {asNonEmptyString(metadata?.cleanupAction) && (
                    <div><span className="text-muted-foreground">{l10n("local.cleanup_01f08357")}{" "}</span><span className="font-mono">{metadata?.cleanupAction as string}</span></div>
                  )}
                </div>
              )}
              {typeof metadata?.created === "boolean" && (
                <div className="text-xs text-muted-foreground">
                  {metadata.created ? l10n("local.created_by_this_run_2ec788a5") : l10n("local.reused_existing_workspace_cb0d01dd")}
                </div>
              )}
              {operation.stderrExcerpt && operation.stderrExcerpt.trim() && (
                <div>
                  <div className="mb-1 text-xs text-red-700 dark:text-red-300">{l10n("local.stderr_excerpt_957d1931")}</div>
              ...
              {operation.stdoutExcerpt && operation.stdoutExcerpt.trim() && (
                <div>
                  <div className="mb-1 text-xs text-muted-foreground">{l10n("local.stdout_excerpt_66ced670")}</div>
    ...
    <div className={cn("agent-settings-content mx-auto max-w-5xl space-y-8", `agent-settings-${activeView}`)}>
      {showLeftAgentNotice ? (
        <div className="flex items-center gap-3 border border-yellow-300/35 bg-yellow-300/10 px-3 py-2 text-sm text-yellow-900 dark:text-yellow-100">
          <p className="min-w-0 flex-1">
            {l10n("local.you_left_this_agent_it_no_longer_appears_in_y_5ad48442")}</p>
          <MembershipAction
            compact
            state="left"
          ...
          <button
            ...
            aria-label={l10n("local.dismiss_agent_membership_notice_e8f6751a")}
      ...
      {pausedEscalationWarning ? (
        <div className="flex items-start gap-3 border border-amber-300/35 bg-amber-300/10 px-3 py-2 text-sm text-amber-900 dark:text-amber-100">
          ...
          <div className="min-w-0 space-y-1">
            <p className="font-medium">{l10n("local.escalation_path_is_paused_314521d9")}</p>
      ...
      {hasInvalidOrgChain ? (
        <div className="flex items-start gap-3 border border-amber-300/35 bg-amber-300/10 px-3 py-2 text-sm text-amber-900 dark:text-amber-100">
          <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" />
          <div className="min-w-0 space-y-1">
            <p className="font-medium">{l10n("local.invalid_reporting_chain_bf9c6d9d")}</p>
            <p className="text-amber-900/90 dark:text-amber-100/90">
              {agent.name} {l10n("local.cannot_accept_tasks_or_start_runs_until_its_r_67219418")}</p>
            ...
            {agent.orgChainHealth?.repairGuidance ? (
              ...
              <p className="text-amber-900/85 dark:text-amber-100/85">
                {l10n("local.assign_this_agent_to_an_active_manager_root_o_d897533c")}</p>
            )}
          </div>
        </div>
      ) : null}
      ...
      <header className="flex flex-wrap items-center justify-between gap-5 border-b border-border pb-6">
        <div className="flex min-w-0 items-center gap-4">
          <div role="img" aria-label={l10n("local.value_avatar_b694db30", {v0: (agent.name)})} className="shrink-0">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <AgentActionButtons
            ...
            assignLabel={l10n("local.assign_task_5dcdad5a")}
            showStatus={false}
            ...
            pauseConfirm={
              builtInState
                ? {
                    title: l10n("local.pause_the_value_157961c1", {v0: (builtInState.definition.displayName)}),
                    description: (
                      <>
                        {builtInFeatureLabel} {l10n("local.depends_on_this_agent_while_paused_e8204fd6")}{" "}
                        {builtInFeatureLabel.toLowerCase()} {l10n("local.generation_is_skipped_and_the_8b2f4962")}{" "}
                        {builtInFeatureLabel} {l10n("local.page_shows_a_warning_f336157c")}</>
                    ),
                  }
                : undefined
            }
          >
            {mobileLiveRun && (
              ...
              >
                ...
                <span className="text-(length:--text-micro) font-medium text-blue-600 dark:text-blue-400">{l10n("local.live_b64ac05f")}</span>
              </Link>
            )}
          </AgentActionButtons>
        </div>
      ...
      {builtInState && (
        <InlineBanner
          ...
          actions={
            ...
            >
              {resetBuiltIn.isPending ? l10n("local.resetting_1505f40d") : l10n("local.reset_to_defaults_e240e635")}
            </Button>
          }
        >
          {l10n("local.ships_with_paperclip_and_powers_f7a276fa")}{" "}<strong>{builtInFeatureLabel}</strong>{l10n("local._configure_it_like_any_agent_model_instructio_c6d76dea")}{" "}{builtInFeatureLabel}.
        </InlineBanner>
      )}
      ...
      {isPendingApproval && (
        <div className="flex flex-wrap items-center gap-3 rounded-md border border-amber-300/60 bg-amber-50 px-3 py-2 text-sm text-amber-900 dark:border-amber-400/40 dark:bg-amber-950/30 dark:text-amber-200">
          <span>{l10n("local.this_agent_is_pending_board_approval_and_cann_241a4b1b")}</span>
          <Button
            variant="outline"
            size="sm"
          ...
          >
            ...
            <span>{l10n("local.approve_agent_65b17eeb")}</span>
          </Button>
        </div>
      )}

      {activeView !== "run-detail" && activeView !== "channels" && <h2 className="text-xl font-semibold">{activeView === "secrets" ? l10n("local.secrets_variables_bd56b51e") : AGENT_DETAIL_NAVIGATION.flatMap(section => section.items).find(item => item.value === activeView)?.label}</h2>}
      ...
      {showConfigActionBar && <footer className="sticky bottom-0 z-10 flex flex-wrap items-center justify-between gap-3 border-t border-border bg-background py-4">
        <p role="status" className="text-xs text-muted-foreground">{configSaving ? l10n("local.saving_changes_39520758") : configDirty ? l10n("local.you_have_unsaved_changes_4e0a4324") : ""}</p>
        <div className="flex gap-2">
          <Button variant="ghost" disabled={!configDirty || configSaving} onClick={() => cancelConfigActionRef.current?.()}>{l10n("local.discard_eb1a70e3")}</Button>
          <Button disabled={!configDirty || configSaving} onClick={() => {Promise.resolve(saveConfigActionRef.current?.()).catch(() => {});}}>{configSaving ? l10n("local.saving_23e39291") : l10n("local.save_changes_dd0ae7a5")}</Button>
        </div>
      </footer>}
      ...
      <div className="flex w-full items-center justify-between">
        ...
        >
          <h3 className="flex items-center gap-2 text-sm font-medium transition-colors hover:text-foreground">
            ...
            <span>{isLive ? l10n("local.live_run_a8a2029f") : l10n("local.latest_run_d1451ce7")}</span>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <section className="rounded-lg border border-border p-4" aria-labelledby="agent-identity-heading">
          <div className="mb-4 flex items-center justify-between gap-3">
            <h3 id="agent-identity-heading" className="text-sm font-medium">{l10n("local.identity_999f23fc")}</h3>
            <StatusBadge status={agent.status} />
          </div>
          <div className="space-y-3">
            <SummaryRow label={l10n("local.role_14736a2e")}><span className="text-sm">{roleLabels[agent.role] ?? agent.role}</span></SummaryRow>
            <SummaryRow label={l10n("local.title_7e8cd205")}><span className="text-sm">{agent.title ?? l10n("local.not_set_4895f731")}</span></SummaryRow>
            <SummaryRow label={l10n("local.reports_to_2b488c09")}>
              {reportsToAgent ? (
                <Link className="text-sm hover:underline" to={agentDetailHref(agentRouteRef(reportsToAgent))}>
                  {reportsToAgent.name}
                </Link>
              ) : <span className="text-sm">{l10n("local.board_4816cbfd")}</span>}
            </SummaryRow>
            <SummaryRow label={l10n("local.direct_reports_48f8714c")}><span className="text-sm tabular-nums">{directReportCount}</span></SummaryRow>
          </div>
        </section>

        <section className="rounded-lg border border-border p-4" aria-labelledby="agent-runtime-heading">
          <div className="mb-4 flex items-center justify-between gap-3">
            <h3 id="agent-runtime-heading" className="text-sm font-medium">{l10n("local.harness_runtime_6711e546")}</h3>
            <Link className="text-xs text-muted-foreground hover:text-foreground" to={agentDetailHref(agentRouteId, "runtime")}>{l10n("local.configure_6defafa2")}</Link>
          </div>
          <div className="space-y-3">
            <SummaryRow label={l10n("local.adapter_0252b849")}><span className="text-sm">{adapterLabels[agent.adapterType] ?? agent.adapterType}</span></SummaryRow>
            <SummaryRow label={l10n("local.model_5e2c614c")}><span className="max-w-64 truncate text-sm font-mono">{configuredModel}</span></SummaryRow>
            <SummaryRow label={l10n("local.session_6959b415")}><span className="max-w-64 truncate text-sm font-mono">{runtimeState?.sessionDisplayId ?? runtimeState?.sessionId ?? l10n("local.no_session_64f06c96")}</span></SummaryRow>
            <SummaryRow label={l10n("local.last_run_512a4821")}>
              <span className="text-sm">{lastRun ? `${lastRun.status} · ${relativeTime(lastRun.createdAt)}` : l10n("local.no_runs_26735edf")}</span>
            </SummaryRow>
          </div>
        </section>

        <section className="rounded-lg border border-border p-4" aria-labelledby="agent-capabilities-heading">
          <h3 id="agent-capabilities-heading" className="mb-3 text-sm font-medium">{l10n("local.capabilities_9460f16a")}</h3>
          {agent.capabilities?.trim() ? (
            <MarkdownBody className="text-sm [&>*:first-child]:mt-0 [&>*:last-child]:mb-0">{agent.capabilities}</MarkdownBody>
          ) : (
            <p className="text-sm text-muted-foreground">{l10n("local.no_capability_summary_has_been_added_ca65ad7b")}</p>
          )}
        </section>

        <section className="rounded-lg border border-border p-4" aria-labelledby="agent-skills-heading">
          <div className="mb-3 flex items-center justify-between gap-3">
            <h3 id="agent-skills-heading" className="text-sm font-medium">{l10n("local.skills_66d0f523")}</h3>
            <Link className="text-xs text-muted-foreground hover:text-foreground" to={agentDetailHref(agentRouteId, "skills")}>{l10n("local.manage_5a234448")}</Link>
          </div>
          {skillNames.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {skillNames.slice(0, 8).map((skill) => <Badge key={skill} variant="secondary">{skill}</Badge>)}
              {skillNames.length > 8 ? <Badge variant="outline">+{skillNames.length - 8} {l10n("local.more_187897ce")}</Badge> : null}
            </div>
          ) : (
            <p className="text-sm text-muted-foreground">{l10n("local.no_skills_enabled_927c400e")}</p>
          )}
        </section>
      </div>
      ...
      <section className="space-y-3" aria-labelledby="agent-recent-tasks-heading">
        <div className="flex items-center justify-between">
          <h3 id="agent-recent-tasks-heading" className="text-sm font-medium">{l10n("local.recent_tasks_49a027dd")}</h3>
          <Link
            to={`/issues?participantAgentId=${agent.id}`}
            className="text-xs text-muted-foreground hover:text-foreground transition-colors"
          >
            {l10n("local.see_all_rarr_71176ed9")}</Link>
        </div>
        {assignedIssues.length === 0 ? (
          <p className="text-sm text-muted-foreground">{l10n("local.no_recent_tasks_e555bbe6")}</p>
        ) : (
          <div className="overflow-hidden rounded-lg border border-border">
            ...
            {assignedIssues.length > 6 && (
              <div className="border-t border-border px-3 py-2 text-center text-xs text-muted-foreground">
                +{assignedIssues.length - 6} {l10n("local.more_tasks_e57109ca")}</div>
            )}
          </div>
        )}
      </section>
      ...
      <section className="space-y-3" aria-labelledby="agent-audit-links-heading">
        <h3 id="agent-audit-links-heading" className="text-sm font-medium">{l10n("local.audit_bb6aea28")}</h3>
  ...
  return (
    <div className="max-w-3xl space-y-3">
      <div className="flex items-baseline justify-between gap-3">
        <h3 className="text-sm font-medium">{l10n("local.configuration_revisions_2aa8f007")}</h3>
        <span className="text-xs text-muted-foreground">{configRevisions?.length ?? 0} {l10n("local.total_11239872")}</span>
      </div>
      {(configRevisions ?? []).length === 0 ? (
        <p className="text-sm text-muted-foreground">{l10n("local.no_configuration_revisions_yet_47fc5637")}</p>
      ) : (
        <div className="space-y-2">
          {(configRevisions ?? []).map((revision) => (
            <div key={revision.id} className="space-y-2 rounded-md border border-border p-3">
              ...
              <p className="text-xs text-muted-foreground">
                {l10n("local.changed_d88b1fed")}{" "}{revision.changedKeys.length > 0 ? revision.changedKeys.join(", ") : l10n("local.no_tracked_changes_83a4a77e")}
              </p>
            </div>
          ))}
        </div>
...
export function ConfigurationTab({
  ...
  const updateAgent = useMutation({
    ...
    onSuccess: (updated) => {
      ...
      pushToast({ title: l10n("local.agent_saved_8ed40b78"), tone: "success" });
    },
    onError: (err) => {
      ...
      const message =
        err instanceof ApiError
          ...
          : err instanceof Error
            ? err.message
            : l10n("local.could_not_save_agent_582fdf69");
      pushToast({ title: l10n("local.save_failed_53ad6f99"), body: message, tone: "error" });
    },
  });
  ...
  const taskAssignHint =
    taskAssignSource === "ceo_role"
      ? l10n("local.enabled_automatically_for_ceo_agents_e7a0e8c2")
      : taskAssignSource === "agent_creator"
        ? l10n("local.enabled_automatically_while_this_agent_can_cr_36da96fc")
        : taskAssignSource === "explicit_grant"
          ? l10n("local.enabled_via_explicit_organization_permission_2ee1a421")
          : taskAssignSource === "simple_default"
            ? l10n("local.enabled_by_simple_organization_wide_task_assi_6b427697")
            : l10n("local.disabled_unless_explicitly_granted_fde881c0");
  ...
  return (
    <div className="agent-settings-form space-y-6">
      ...
      {content === "permissions" ? <div>
        <h3 className="text-sm font-medium mb-3">{l10n("local.permissions_abccc78c")}</h3>
        <div className="border border-border rounded-lg p-4 space-y-4">
          <div className="flex items-center justify-between gap-4 text-sm">
            <div className="space-y-1">
              <div>{l10n("local.can_create_new_agents_55d2d619")}</div>
              <p className="text-xs text-muted-foreground">
                {l10n("local.lets_this_agent_create_or_hire_agents_this_al_90b2e413")}</p>
            </div>
            <ToggleSwitch
          ...
          <div className="flex items-center justify-between gap-4 text-sm">
            <div className="space-y-1">
              <div>{l10n("local.can_create_import_skills_77c0a376")}</div>
              <p className="text-xs text-muted-foreground">
                {l10n("local.lets_this_agent_install_import_create_and_sca_4541d906")}</p>
            </div>
            <ToggleSwitch
          ...
          <div className="flex items-center justify-between gap-4 text-sm">
            <div className="space-y-1">
              <div>{l10n("local.can_assign_tasks_05d8436a")}</div>
              <p className="text-xs text-muted-foreground">
                {taskAssignHint}
              </p>
            </div>
    ...
    return (
      <div className="max-w-3xl">
        <p className="text-sm text-muted-foreground">
          {l10n("local.instructions_bundles_are_only_available_for_l_ac25d90e")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {showSaveNotice && <p className="text-xs text-muted-foreground">
        {l10n("local.saved_instructions_affect_the_next_run_active_2bd6467d")}</p>}
            ...
            <div className="grid gap-x-6 gap-y-4 md:grid-cols-(--gtc-18)">
              <label className="space-y-1.5 min-w-0">
                <span className="text-xs font-medium text-muted-foreground flex items-center gap-1">
                  {l10n("local.mode_5e23ec6a")}<Tooltip>
                    <TooltipTrigger asChild>
                      <HelpCircle className="h-3 w-3 text-muted-foreground cursor-help" />
                    </TooltipTrigger>
                    <TooltipContent side="right" sideOffset={4}>
                      {l10n("local.managed_paperclip_stores_and_serves_the_instr_1f6587a4")}</TooltipContent>
                  </Tooltip>
                </span>
                <div className="flex gap-2">
                  <Button
                  ...
                  >
                    {l10n("local.managed_8f2de600")}</Button>
                  <Button
                    type="button"
                    size="sm"
                  ...
                  >
                    {l10n("local.external_68c114ea")}</Button>
                </div>
              </label>
              <label className="space-y-1.5 min-w-0">
                <span className="text-xs font-medium text-muted-foreground flex items-center gap-1">
                  {l10n("local.root_path_c14a3db2")}<Tooltip>
                    <TooltipTrigger asChild>
                      <HelpCircle className="h-3 w-3 text-muted-foreground cursor-help" />
                    </TooltipTrigger>
                    <TooltipContent side="right" sideOffset={4}>
                      {l10n("local.the_absolute_directory_on_disk_where_the_inst_8e07f676")}</TooltipContent>
                  </Tooltip>
                </span>
                {currentMode === "managed" ? (
                  <div className="flex items-center gap-1.5 font-mono text-xs text-muted-foreground pt-1.5">
                    <span className="min-w-0 truncate" title={currentRootPath || undefined}>{currentRootPath || l10n("local._managed_ffdb0bcd")}</span>
                    {currentRootPath && (
              ...
              <label className="space-y-1.5">
                <span className="text-xs font-medium text-muted-foreground flex items-center gap-1">
                  {l10n("local.entry_file_134ed587")}<Tooltip>
                    <TooltipTrigger asChild>
                      <HelpCircle className="h-3 w-3 text-muted-foreground cursor-help" />
                    </TooltipTrigger>
                    <TooltipContent side="right" sideOffset={4}>
                      {l10n("local.the_main_file_the_agent_reads_first_when_load_57604ed3")}</TooltipContent>
                  </Tooltip>
                </span>
                <Input
      ...
      >
        <div className={cn(
          ...
          <div className="flex items-center justify-between">
            <h4 className="text-sm font-medium">{l10n("local.files_abc7e989")}</h4>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <span className="ml-3 shrink-0 rounded border border-amber-500/40 bg-amber-500/10 text-amber-800 dark:text-amber-200 px-1.5 py-0.5 text-(length:--text-nano) uppercase tracking-wide cursor-help">
                        {l10n("local.virtual_file_e0e99522")}</span>
                    </TooltipTrigger>
                    <TooltipContent side="right" sideOffset={4}>
                      {l10n("local.legacy_inline_prompt_this_deprecated_virtual_767eebd8")}</TooltipContent>
                  </Tooltip>
                );
              }
              return (
                <span className="ml-3 shrink-0 rounded border border-border text-muted-foreground px-1.5 py-0.5 text-(length:--text-nano) uppercase tracking-wide">
                  {file.isEntryFile ? l10n("local.entry_923fe539") : `${file.size}b`}
                </span>
              );
            }}
          />
                ...
                <p className="text-xs text-muted-foreground">
                  {selectedFileExists
                    ? selectedFileSummary?.deprecated
                      ? l10n("local.deprecated_virtual_file_14230ef1")
                      : l10n("local.value_file_9513645a", {v0: (selectedFileDetail?.language ?? "text")})
                    : l10n("local.new_file_in_this_bundle_3eb813e1")}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <div className="flex items-center rounded-md border border-border p-0.5" role="group" aria-label={l10n("local.instruction_file_view_348b6a6a")}>
              ...
              {!fileLoading && (
                <CopyText
                  text={displayValue}
                  ariaLabel={l10n("local.copy_instructions_file_as_markdown_8b3e0eed")}
                  title={l10n("local.copy_as_markdown_fec6709d")}
                  copiedLabel={l10n("local.copied_8d525e5f")}
              ...
              {selectedFileExists && !selectedFileSummary?.deprecated && selectedOrEntryFile !== currentEntryFile && (
                <Button
                  ...
                  variant="outline"
                  onClick={() => {
                    if (confirm(l10n("local.delete_value_a19801bb", {v0: (selectedOrEntryFile)}))) {
          ...
          {selectedFileExists && fileLoading && !selectedFileDetail ? (
            ...
            <div className="min-h-(--sz-420px) rounded-md border border-border bg-background p-4">
              {displayValue.trim() ? (
                ...
                <p className="text-sm text-muted-foreground">{l10n("local.this_instruction_file_is_empty_51435439")}</p>
              )}
            </div>
          ) : instructionMode === "raw" ? (
            <pre
            ...
            >
              <MarkdownEditor
                ...
                placeholder={l10n("local._agent_instructions_b4a3525f")}
            ...
            <textarea
              aria-label={l10n("local.instruction_file_editor_c5c43562")}
              value={displayValue}
              ...
              placeholder={l10n("local.file_contents_69423bab")}
            />
          )}
        </div>
      </div>
    ...
    >
      ...
      {(metrics.totalTokens > 0 || metrics.cost > 0) && (
        <div className="flex items-center gap-2 pl-5.5 text-(length:--text-micro) text-muted-foreground tabular-nums">
          {metrics.totalTokens > 0 && <span>{formatTokens(metrics.totalTokens)} {l10n("local.tok_1a7674eb")}</span>}
...
function RunsTab({
  ...
  if (runs.length === 0) {
    return <p className="text-sm text-muted-foreground">{l10n("local.no_runs_yet_72a85758")}</p>;
  }
      ...
      return (
        <div className="space-y-3 min-w-0 overflow-x-hidden">
          ...
          >
            ...
            {l10n("local.back_to_runs_5f364113")}</Link>
  ...
  return (
    <div className="space-y-4 min-w-0">
        ...
        <div className="flex flex-col sm:flex-row">
          ...
          <div className="flex-1 p-4 space-y-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {(run.status === "running" || run.status === "queued") && (
                ...
                >
                  {cancelRun.isPending ? l10n("local.cancelling_91b104db") : l10n("local.cancel_19766ed6")}
                </Button>
              )}
              {canResumeLostRun && (
                <Button
                ...
                >
                  ...
                  {resumeRun.isPending ? l10n("local.resuming_c494e3ca") : l10n("local.resume_d640c742")}
                </Button>
              )}
              {canRetryRun && !canResumeLostRun && (
                ...
                >
                  <RotateCcw className="h-3.5 w-3.5 mr-1" />
                  {retryRun.isPending ? l10n("local.retrying_a16c8b1c") : l10n("local.retry_942087cc")}
                </Button>
              )}
              <Button
                variant="ghost"
              ...
              {canUseProviderTrace && !["queued", "running"].includes(run.status) ? (
                ...
                >
                  <RotateCcw className="h-3.5 w-3.5 mr-1" />
                  {rerunWithTrace.isPending ? l10n("local.starting_bbe5fc3b") : l10n("local.re_run_with_provider_trace_296d058e")}
                </Button>
              ) : null}
            </div>
            ...
            {run.responsibleUserId && (
              ...
              >
                {l10n("local.on_behalf_of_cdccf734")}{" "}
            ...
            {Boolean(run.identityHistory?.length) && (
              <details className="text-xs text-muted-foreground" data-testid="run-identity-history">
                <summary className="cursor-pointer">{l10n("local.github_identity_history_2d754cf1")}</summary>
                <ol className="mt-2 space-y-2">
                  {run.identityHistory!.map((identity) => {
                    ...
                    return (
                      <li key={identity.id}>
                        <span className="text-foreground">{person?.user?.name ?? person?.user?.email ?? identity.responsibleUserId ?? l10n("local.no_responsible_person_409926df")}</span>
                        {" · "}{identity.cause}{" · "}{identity.status}
                        {identity.github ? (
                          <span className="block">
                            {identity.github.login ? (l10n("local._value_8d7c421d", {v0: (identity.github.login)}) + " ") : ""}
                            {identity.github.source ? (l10n("local.value_33b1db25", {v0: (identity.github.source)}) + " ") : ""}
                            {identity.github.status}{identity.github.reason ? `: ${identity.github.reason}` : ""}
                          </span>
                        ) : <span className="block">{l10n("local.no_github_operation_recorded_5e5bfca3")}</span>}
                      </li>
                    );
                  })}
                </ol>
            ...
            {resumeRun.isError && (
              <div className="text-xs text-destructive">
                {resumeRun.error instanceof Error ? resumeRun.error.message : l10n("local.failed_to_resume_run_56873547")}
              </div>
            )}
            {retryRun.isError && (
              <div className="text-xs text-destructive">
                {retryRun.error instanceof Error ? retryRun.error.message : l10n("local.failed_to_retry_run_0ce80ccb")}
              </div>
            )}
            {startTime && (
              <div className="space-y-0.5">
                ...
                {displayDurationSec !== null && (
                  <div className="text-xs text-muted-foreground">
                    {l10n("local.duration_298d6c75")}{" "}{displayDurationSec >= 60 ? l10n("local.valuem_values_778616d1", {v0: (Math.floor(displayDurationSec / 60)), v1: (displayDurationSec % 60)}) : l10n("local.values_7680eb34", {v0: (displayDurationSec)})}
                  </div>
                )}
              </div>
            )}
            ...
            {run.errorCode === "claude_auth_required" && adapterType === "claude_local" && (
              <div className="space-y-2">
                ...
                >
                  {runClaudeLogin.isPending ? l10n("local.running_claude_login_58ac84a4") : l10n("local.login_to_claude_code_5814b863")}
                </Button>
                {runClaudeLogin.isError && (
                  <p className="text-xs text-destructive">
                    {runClaudeLogin.error instanceof Error
                      ? runClaudeLogin.error.message
                      : l10n("local.failed_to_run_claude_login_130ad905")}
                  </p>
                )}
                {claudeLoginResult?.loginUrl && (
                  <p className="text-xs">
                    {l10n("local.login_url_53aff5fe")}<a
            ...
            {hasNonZeroExit && (
              <div className="text-xs text-red-600 dark:text-red-400">
                {l10n("local.exit_code_ccc6eb1c")}{" "}{run.exitCode}
                {run.signal && <span className="text-muted-foreground ml-1">{l10n("local._signal_145327d0")}{" "}{run.signal})</span>}
              </div>
            )}
            {retryState && (
          ...
          {hasMetrics && (
            <div className="border-t sm:border-t-0 sm:border-l border-border p-4 grid grid-cols-2 gap-x-4 sm:gap-x-8 gap-y-3 content-center tabular-nums">
              <div>
                <div className="text-xs text-muted-foreground">{l10n("local.input_36ecb4f8")}</div>
                <div className="text-sm font-medium font-mono">{formatTokens(metrics.input)}</div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">{l10n("local.output_b2439bcb")}</div>
                <div className="text-sm font-medium font-mono">{formatTokens(metrics.output)}</div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">{l10n("local.cached_a251c182")}</div>
                <div className="text-sm font-medium font-mono">{formatTokens(metrics.cached)}</div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">{l10n("local.cost_204a5eb2")}</div>
        ...
        {hasSession && (
          <div className="border-t border-border">
            ...
            >
              ...
              {l10n("local.session_6959b415")}{sessionChanged && <span className="text-yellow-400 ml-1">{l10n("local._changed_5311f2a8")}</span>}
            </button>
            {sessionOpen && (
              <div className="px-4 pb-3 space-y-1 text-xs">
                {run.sessionIdBefore && (
                  <div className="flex items-center gap-2">
                    <span className="text-muted-foreground w-12">{sessionChanged ? l10n("local.before_9bb72500") : "ID"}</span>
                    <CopyText text={run.sessionIdBefore} className="font-mono" />
                  </div>
                )}
                {sessionChanged && run.sessionIdAfter && (
                  <div className="flex items-center gap-2">
                    <span className="text-muted-foreground w-12">{l10n("local.after_7b68fe55")}</span>
                  ...
                  <div className="pt-1">
                    <button
                      ...
                      onClick={() => {
                        ...
                        const confirmed = window.confirm(
                          l10n("local.clear_session_for_value_issuevalue_touched_by_ad2c1736", {v0: (issueCount), v1: (englishPluralSuffix(issueCount === 1 ? "" : "s"))}),
                        );
                        if (!confirmed) return;
                    ...
                    >
                      {clearSessionsForTouchedIssues.isPending
                        ? l10n("local.clearing_session_7a70b1ef")
                        : l10n("local.clear_session_for_these_tasks_4e1f6dad")}
                    </button>
                    {clearSessionsForTouchedIssues.isError && (
                      <p className="text-(length:--text-micro) text-destructive mt-1">
                        {clearSessionsForTouchedIssues.error instanceof Error
                          ...
                          : l10n("local.failed_to_clear_sessions_ed9e6fe6")}
                      </p>
                    )}
                  </div>
                )}
      ...
      {touchedIssues && touchedIssues.length > 0 && (
        <div className="space-y-2">
          <span className="text-xs font-medium text-muted-foreground">{l10n("local.tasks_touched_56b756a2")}{touchedIssues.length})</span>
      ...
      {run.stdoutExcerpt && !run.logRef && (
        <div className="space-y-1">
          <span className="text-xs font-medium text-muted-foreground">{l10n("local.stdout_63d42d26")}</span>
...
export function LogViewer({ run, adapterType }: { run: HeartbeatRun; adapterType: string }) {
  ...
  if (loading && logLoading) {
    return <p className="text-xs text-muted-foreground">{l10n("local.loading_run_logs_b719a304")}</p>;
  }

  if (events.length === 0 && logLines.length === 0 && !logError) {
    return <p className="text-xs text-muted-foreground">{l10n("local.no_log_events_d1e81320")}</p>;
  }
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <span className="text-xs font-medium text-muted-foreground">
          {l10n("local.transcript_fd6a2ef4")}{transcript.length})
        </span>
        <div className="flex items-center gap-2">
          ...
          {isLive && !isFollowing && (
            ...
            >
              {l10n("local.jump_to_live_ce39c3a6")}</Button>
          )}
          {isLive && (
      ...
      <div className="max-h-(--sz-38rem) overflow-y-auto rounded-2xl border border-border/70 bg-background/40 p-3 sm:p-4">
        <RunTranscriptView
          ...
          emptyMessage={run.logRef ? l10n("local.waiting_for_transcript_8f7d4ad6") : l10n("local.no_persisted_transcript_for_this_run_e8b062b8")}
        />
        {hasMoreLog && (
          <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-border/60 pt-3">
            ...
            >
              {loadingMoreLog ? l10n("local.loading_47d2a515") : l10n("local.load_more_log_43baac37")}
            </Button>
            <span className="text-xs text-muted-foreground">
              {l10n("local.showing_the_first_7e92a4e1")}{" "}{Math.round(logOffset / 1024).toLocaleString("en-US")} {l10n("local.kb_59281a6b")}{typeof run.logBytes === "number" && run.logBytes > 0
                ? (" " + l10n("local.of_value_kb_94fcf66e", {v0: (Math.round(run.logBytes / 1024).toLocaleString("en-US"))}))
                : ""}
            </span>
          </div>
        )}
      ...
      {(run.status === "failed" || run.status === "timed_out") && (
        <div className="rounded-lg border border-red-300 dark:border-red-500/30 bg-red-50 dark:bg-red-950/20 p-3 space-y-2">
          <div className="text-xs font-medium text-red-700 dark:text-red-300">{l10n("local.failure_details_6317e0c6")}</div>
          {run.error && (
            <div className="text-xs text-red-600 dark:text-red-200">
              <span className="text-red-700 dark:text-red-300">{l10n("local.error_61706290")}{" "}</span>
              {redactPathText(run.error, censorUsernameInLogs)}
            </div>
          )}
          {run.stderrExcerpt && run.stderrExcerpt.trim() && (
            <div>
              <div className="text-xs text-red-700 dark:text-red-300 mb-1">{l10n("local.stderr_excerpt_957d1931")}</div>
          ...
          {run.resultJson && (
            <div>
              <div className="text-xs text-red-700 dark:text-red-300 mb-1">{l10n("local.adapter_result_json_4774d3f7")}</div>
              <pre className="bg-red-50 dark:bg-neutral-950 rounded-md p-2 text-xs overflow-x-auto whitespace-pre-wrap text-red-800 dark:text-red-100">
          ...
          {run.stdoutExcerpt && run.stdoutExcerpt.trim() && !run.resultJson && (
            <div>
              <div className="text-xs text-red-700 dark:text-red-300 mb-1">{l10n("local.stdout_excerpt_66ced670")}</div>
              <pre className="bg-red-50 dark:bg-neutral-950 rounded-md p-2 text-xs overflow-x-auto whitespace-pre-wrap text-red-800 dark:text-red-100">
      ...
      {events.length > 0 && (
        <div>
          <div className="mb-2 text-xs font-medium text-muted-foreground">{l10n("local.events_6e9b7c69")}{events.length})</div>
    ...
    <div className="space-y-6">
      ...
      {newToken && (
        <div className="border border-yellow-300 dark:border-yellow-600/40 bg-yellow-50 dark:bg-yellow-500/5 rounded-lg p-4 space-y-2">
          <p className="text-sm font-medium text-yellow-700 dark:text-yellow-400">
            {l10n("local.api_key_created_copy_it_now_it_will_not_be_sh_37fc2c91")}</p>
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              title={tokenVisible ? l10n("local.hide_ac20a57b") : l10n("local.show_0df6f1ca")}
            >
            ...
            {copied && <span className="text-xs text-green-400">{l10n("local.copied_ea61bc15")}</span>}
          </div>
          <Button
            variant="ghost"
            size="sm"
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        <h3 className="text-xs font-medium text-muted-foreground flex items-center gap-2">
          ...
          {l10n("local.create_api_key_e133f431")}</h3>
        <p className="text-xs text-muted-foreground">
          {l10n("local.api_keys_allow_this_agent_to_authenticate_cal_aaab6905")}</p>
        <div className="flex items-center gap-2">
          <Input
            placeholder={l10n("local.key_name_e_g_production_72bae87b")}
            value={newKeyName}
      ...
      {/* Active keys */}
      {isLoading && <p className="text-sm text-muted-foreground">{l10n("local.loading_keys_e0c65255")}</p>}

      {!isLoading && activeKeys.length === 0 && !newToken && (
        <p className="text-sm text-muted-foreground">{l10n("local.no_active_api_keys_de64e83d")}</p>
      )}

      {activeKeys.length > 0 && (
        <div>
          <h3 className="text-xs font-medium text-muted-foreground mb-2">
            {l10n("local.active_keys_140448ef")}</h3>
          <div className="border border-border rounded-lg divide-y divide-border">
              ...
              <div key={key.id} className="flex items-center justify-between px-4 py-2.5">
                <div>
                  ...
                  <span className="text-xs text-muted-foreground ml-3">
                    {l10n("local.created_d70b9e24")}{" "}{formatDate(key.createdAt)}
                  </span>
                </div>
                <Button
                  variant="ghost"
      ...
      {revokedKeys.length > 0 && (
        <div>
          <h3 className="text-xs font-medium text-muted-foreground mb-2">
            {l10n("local.revoked_keys_eee9ab4f")}</h3>
            ...
            {revokedKeys.map((key: AgentKey) => (
              <div key={key.id} className="flex items-center justify-between px-4 py-2.5">
                <div>
                  ...
                  <span className="text-xs text-muted-foreground ml-3">
                    {l10n("local.revoked_f6f738d0")}{" "}{key.revokedAt ? formatDate(key.revokedAt) : ""}
                  </span>
                </div>
              </div>
            ))}
