...
function CaseRelationshipsSection({
  ...
  return (
    <section className="space-y-3" aria-label={l10n("local.case_relationships_8d5d2b25")}>
      {parent ? (
        <div className="space-y-1">
          <h2 className="text-xs font-medium text-muted-foreground">{l10n("local.parent_5f7953f7")}</h2>
          <CaseChildrenTree children={[parent]} />
        </div>
      ) : null}
      {children.length > 0 ? (
        <div className="space-y-1">
          <h2 className="text-xs font-medium text-muted-foreground">{l10n("local.children_3583a358")}{" "}{children.length}</h2>
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="sm" className="h-6 gap-1 px-2 text-xs text-muted-foreground">
          <Plus className="h-3.5 w-3.5" /> {l10n("local.labels_934b8899")}</Button>
      </PopoverTrigger>
      <PopoverContent align="start" className="w-64 p-2">
        <Input
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          placeholder={l10n("local.search_labels_6baf3082")}
        ...
        <div className="max-h-52 space-y-0.5 overflow-y-auto">
          ...
          {filtered.length === 0 && !search.trim() && (
            <p className="px-2 py-1 text-xs text-muted-foreground">{l10n("local.no_labels_yet_a3a46c67")}</p>
          )}
        </div>
        {search.trim() && !all.some((l) => l.name.toLowerCase() === search.trim().toLowerCase()) && (
          <div className="mt-2 flex items-center gap-2 border-t border-border pt-2">
            <input
              ...
              aria-label={l10n("local.new_label_color_a4b3896f")}
            />
            <Button
              size="sm"
              variant="secondary"
            ...
            >
              {l10n("local.create_e52488af")}{search.trim()}”
            </Button>
          </div>
        )}
      </PopoverContent>
...
function CasePropertiesContent({
  ...
  return (
    <div className={cn("space-y-4", isFull && "space-y-6")}>
      <PropertySection title={l10n("local.case_15f86e24")} first>
        <CasePropertyRow label={l10n("local.type_baaddf70")} mode={mode}>
          <PropertyChip>{caseData.caseType}</PropertyChip>
        </CasePropertyRow>
        {caseData.key ? (
          <CasePropertyRow label={l10n("local.key_99a52df3")} mode={mode}>
            <CaseCopyableToken
              value={caseData.key}
        ...
        <CasePropertyRow label={l10n("local.labels_934b8899")} wrap mode={mode}>
          ...
          {labelsPending ? <span className="text-xs text-muted-foreground">{l10n("local.saving_dc85af8f")}</span> : null}
        </CasePropertyRow>
      </PropertySection>

      {propertyRows.length > 0 ? (
        <PropertySection title={l10n("local.fields_616f48c1")}>
      ...
      <PropertySection title={l10n("local.linked_tasks_f0b70ce5")}>
        {caseData.issueLinks.length === 0 ? (
          <CasePropertyRow label={l10n("local.tasks_b3a60e61")} mode={mode}>
            <span className="text-xs text-muted-foreground">{l10n("local.none_yet_adb85cf2")}</span>
          </CasePropertyRow>
        ) : (
          <CasePropertyRow label={l10n("local.tasks_b3a60e61")} wrap mode={mode}>
      ...
      <PropertySection title={l10n("local.childrenvalue_03441c5a", {v0: (childCases.length > 0 ? ` ${childCases.length}` : "")})}>
        <CaseChildrenTree children={childCases} />
      </PropertySection>

      {caseData.attachments.length > 0 ? (
        <PropertySection title={l10n("local.attachments_634de114")}>
          <CasePropertyRow label={l10n("local.files_abc7e989")} mode={mode}>
            <span className="text-xs text-muted-foreground">
              {caseData.attachments.length} {caseData.attachments.length === 1 ? l10n("local.file_3b9c358f") : l10n("local.files_3d7db37d")}
            </span>
          </CasePropertyRow>
        </PropertySection>
      ) : null}
...
export function CaseDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: l10n("local.cases_2249bd50"), href: caseHref() },
      { label: caseData ? `${caseData.identifier} — ${caseData.title}` : (caseIdentifier ?? l10n("local.case_15f86e24")) },
    ]);
  ...
  if (caseQuery.isError || !caseData) {
    return (
      <div className="mx-auto max-w-md py-16 text-center">
        <p className="text-sm text-muted-foreground">{l10n("local.case_not_found_70f7c9a4")}</p>
        <Link to={caseHref()} className="mt-2 inline-block text-sm text-primary hover:underline">
          {l10n("local._back_to_cases_c3f279a2")}</Link>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      <header className="space-y-3">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex shrink-0 items-center gap-1">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="ghost" size="icon-xs" aria-label={l10n("local.more_case_actions_1b33c141")} title={l10n("local.more_case_actions_1b33c141")}>
      ...
      <Tabs defaultValue="overview" className="space-y-4">
        <TabsList variant="line" className="w-full justify-start gap-1">
          ...
          <TabsTrigger value="properties">{l10n("local.properties_ae43692b")}</TabsTrigger>
          <TabsTrigger value="activity">
            {l10n("local.activity_38da1505")}{events.length > 0 && <span className="ml-1 text-muted-foreground">{events.length}</span>}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          {description ? (
            <section className="space-y-2">
              <h2 className="text-sm font-semibold">{l10n("local.description_526e0087")}</h2>
          ...
          {caseData.attachments.length > 0 && (
            <section className="space-y-2">
              <h2 className="text-sm font-semibold">{l10n("local.attachments_34af4fbe")}{caseData.attachments.length})</h2>
