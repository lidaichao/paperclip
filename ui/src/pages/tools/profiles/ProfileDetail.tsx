...
export function ProfileDetail({
  ...
  const updateProfile = useMutation({
    ...
    onError: (error: unknown) => pushToast({ title: l10n("local.could_not_update_profile_275119d2"), body: errorBody(error), tone: "error" }),
  });
  ...
  const duplicateProfile = useMutation({
    ...
    onError: (error: unknown) => pushToast({ title: l10n("local.could_not_duplicate_552d5a1a"), body: errorBody(error), tone: "error" }),
  });
  ...
  const removeAssignment = useMutation({
    ...
    onSuccess: () => {
      ...
      invalidate();
      pushToast({ title: l10n("local.assignment_removed_65635b6d"), tone: "success" });
    },
    onError: (error: unknown) => pushToast({ title: l10n("local.could_not_remove_assignment_37b173e6"), body: errorBody(error), tone: "error" }),
  });
  ...
  const reviewNewTools = useMutation({
    ...
    onSuccess: () => {
      ...
      pushToast({ title: l10n("local.new_tools_reviewed_c31d721b"), tone: "success" });
    },
    onError: (error: unknown) => pushToast({ title: l10n("local.could_not_submit_review_c3272c49"), body: errorBody(error), tone: "error" }),
  });

  if (data.profiles.isLoading) return <LoadingState label={l10n("local.loading_profile_4a47f12d")} />;
  if (data.profiles.isError) return <ErrorState error={data.profiles.error} onRetry={() => data.profiles.refetch()} />;
  if (!profile) {
    return (
      <div className="space-y-4">
        <ToolsPageHeader title={l10n("local.profile_not_found_99993072")} description={l10n("local.this_access_profile_may_have_been_deleted_86548606")} />
        <Button variant="outline" onClick={() => navigate("/apps/advanced/profiles")}>{l10n("local.back_to_profiles_657ca845")}</Button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <ToolsPageHeader
        title={profile.name}
        description={profile.description ?? l10n("local.no_description_yet_6d962a3d")}
        actions={
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="outline" disabled={archived} onClick={() => setDialog("duplicate")}>
              ...
              {l10n("local.duplicate_02cdaabf")}</Button>
            {archived ? (
              <Button variant="outline" onClick={() => setDialog("restore")}>
                <ArchiveRestore className="mr-1.5 h-4 w-4" />
                {l10n("local.restore_a76e13b9")}</Button>
            ) : (
              <Button variant="outline" onClick={() => setDialog("archive")}>{l10n("local.archive_66f4804e")}</Button>
            )}
      ...
      <div className="flex flex-wrap items-center gap-3 text-sm">
        ...
        <span className="text-muted-foreground">{l10n("local.updated_3a5ecca1")}{" "}<RelativeTime value={profile.updatedAt} /></span>
      ...
      {created ? (
        <div className="flex items-center justify-between gap-3 rounded-lg border border-primary/30 bg-primary/5 px-4 py-3">
          <div>
            <p className="text-sm font-medium text-foreground">{l10n("local.profile_saved_ea278dbc")}</p>
            <p className="text-sm text-muted-foreground">
              {unassigned ? l10n("local.assign_it_to_agents_before_it_changes_their_a_ac3e7b4e") : l10n("local.assignments_are_active_now_ae99b4e1")}
            </p>
          </div>
          <div className="flex gap-2">
            <Button size="sm" onClick={() => navigate(`/apps/advanced/profiles/${profile.id}/edit?step=3`)}>
              {l10n("local.assign_8ece895c")}</Button>
      ...
      {archived ? (
        <div className="rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-950">
          {l10n("local.this_profile_is_archived_it_does_not_apply_to_dc808dda")}</div>
      ) : null}
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-base font-semibold text-foreground">{l10n("local.what_it_allows_ce76de74")}</h2>
          <Button variant="outline" size="sm" disabled={archived} onClick={() => navigate(`/apps/advanced/profiles/${profile.id}/edit?step=2`)}>
            {l10n("local.edit_tools_49794232")}</Button>
        </div>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-base font-semibold text-foreground">{l10n("local.who_has_it_e38b5552")}</h2>
      ...
      <section className="space-y-3">
        <h2 className="text-base font-semibold text-foreground">{l10n("local.new_tools_that_appear_later_ffe66368")}</h2>
        <NewToolsSetting
      ...
      <Button variant="link" className="h-auto px-0" onClick={() => navigate("/apps/advanced/profiles?check=1")}>
        ...
        {l10n("local.check_what_an_agent_can_actually_do_587f7c0e")}</Button>
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-950">
      <div>
        <p className="font-medium">
          {loading ? l10n("local.new_tools_need_review_c656c4a6") : l10n("local.value_added_value_new_value_since_your_last_r_6471d6f1", {v0: (appLabel), v1: (count), v2: (count === 1 ? "tool" : "tools")})}
        </p>
        <p className="text-amber-900/80">{l10n("local.choose_which_ones_this_profile_should_allow_0ceff7b6")}</p>
      </div>
      <Button size="sm" onClick={onReview}>{l10n("local.review_aff0766a")}</Button>
    </div>
  );
}
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{l10n("local.review_new_tools_013d6674")}</DialogTitle>
          <DialogDescription>
            {l10n("local.allow_the_tools_this_profile_should_use_keep_7d8cb98b")}</DialogDescription>
        </DialogHeader>
        {loading ? (
          <LoadingState label={l10n("local.loading_new_tools_f78da714")} />
        ) : error ? (
          <ErrorState error={error} onRetry={onRetry} />
        ) : tools.length === 0 ? (
          <div className="rounded-lg border border-dashed border-border px-4 py-6 text-sm text-muted-foreground">
            {l10n("local.there_are_no_new_tools_waiting_for_review_03570962")}</div>
        ) : (
              ...
              <div key={tool.catalogEntryId} className="grid gap-3 px-3 py-3 sm:grid-cols-[1fr_auto]">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    {tool.applicationName ?? tool.connectionName ?? l10n("local.app_tool_94f7dde1")} {l10n("local._added_ab2ec5e4")}{" "}{formatShortDate(tool.addedAt)}
                  </p>
                </div>
                <div className="flex items-center gap-2 sm:justify-end">
                  <label className="inline-flex items-center gap-1.5 text-sm">
                    ...
                    />
                    {l10n("local.allow_e213c161")}</label>
                  <label className="inline-flex items-center gap-1.5 text-sm">
                    <input
                      type="radio"
                      name={`review-${tool.catalogEntryId}`}
                    ...
                    />
                    {l10n("local.keep_blocked_bbc57309")}</label>
                </div>
              </div>
            ))}
          </div>
        ...
        <DialogFooter>
          ...
          <Button disabled={pending || loading || tools.length === 0} onClick={onSubmit}>
            {l10n("local.submit_review_cb81e6b1")}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
function AllowList({ rows, total }: { rows: AllowRow[]; total: number }) {
  if (rows.length === 0) {
    return (
      <div className="rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-950">
        {l10n("local.this_profile_allows_0_tools_agents_with_only_2977644d")}</div>
    );
  }
  return (
    <div className="overflow-hidden rounded-lg border border-border">
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-border bg-muted/40 text-left text-xs font-medium text-muted-foreground">
            ...
            <th className="px-3 py-2 font-medium">{l10n("local.app_0d04bfeb")}</th>
            <th className="px-3 py-2 font-medium">{l10n("local.capabilities_9460f16a")}</th>
                ...
                <div className="flex flex-col">
                  ...
                  {row.degraded ? (
                    <a className="mt-0.5 inline-flex items-center gap-1 text-xs font-medium text-primary hover:underline" href={`/apps/${row.connectionId}`}>
                      ...
                      {l10n("local.reconnect_bf8a9eab")}</a>
                  ) : null}
                </div>
              </td>
              <td className="px-3 py-2">
                <span>{row.app}</span>
                {row.degraded ? <span className="ml-2 text-xs text-muted-foreground">{row.app} {l10n("local.is_disconnected_39c20965")}</span> : null}
              </td>
              ...
              <td className="px-3 py-2">
                ...
                {row.autoAddedAt ? (
                  <div className="mt-0.5 text-xs text-muted-foreground">
                    {l10n("local.added_automatically_9ef991ce")}{" "}{formatShortDate(row.autoAddedAt)}
                  </div>
                ) : null}
              </td>
            </tr>
      ...
      {rows.length > 80 ? (
        <p className="border-t border-border px-3 py-2 text-xs text-muted-foreground">
          {l10n("local.showing_80_of_3375f9a2")}{" "}{rows.length} {l10n("local.allowed_tools_a421da5c")}</p>
      ) : (
        <p className="border-t border-border px-3 py-2 text-xs text-muted-foreground">
          {l10n("local.allows_8b1d52d2")}{" "}{rows.length} {l10n("local.of_28391d3b")}{" "}{total} {l10n("local.known_tools_f6ff6524")}</p>
      )}
    </div>
  );
}
  ...
  if (profile.bindings.length === 0) {
    return (
      <div className="rounded-lg border border-dashed border-border px-4 py-5">
        <p className="text-sm font-medium text-foreground">{l10n("local.not_assigned_yet_b988b42c")}</p>
        <p className="text-sm text-muted-foreground">{l10n("local.assign_this_profile_before_it_changes_access_2836f7e1")}</p>
      </div>
    );
  }
  return (
        ...
        <div key={binding.id} className="flex items-center justify-between gap-3 px-3 py-2.5">
          <div className="flex min-w-0 items-center gap-3">
            <span className="flex h-8 w-8 items-center justify-center rounded-full bg-muted text-xs font-semibold text-muted-foreground">
              {binding.targetType === "company" ? l10n("local.or_0568bd03") : binding.targetType.slice(0, 2).toUpperCase()}
            </span>
            <div className="min-w-0">
...
function NewToolsSetting({
  ...
  const options: Array<{ value: ToolProfileDefaultAction; title: string; body: string }> = [
    { value: "deny", title: l10n("local.stay_blocked_until_reviewed_816eabb2"), body: "New tools do not become available automatically." },
    { value: "allow", title: l10n("local.allowed_automatically_7fed1ac8"), body: "New tools from selected apps become available right away." },
  ];
  return (
  ...
  if (kind === "edit") {
    return (
      <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{l10n("local.edit_profile_15c4aa13")}</DialogTitle>
            <DialogDescription>{l10n("local.update_the_profile_name_and_description_44e3c158")}</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-1.5">
              <Label htmlFor="edit-profile-name">{l10n("local.name_dcd1d522")}</Label>
              <Input id="edit-profile-name" value={name} onChange={(e) => setName(e.target.value)} />
              {duplicateName ? <p className="text-xs text-destructive">{l10n("local.another_profile_already_uses_this_name_023eb0b1")}</p> : null}
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="edit-profile-description">{l10n("local.description_526e0087")}</Label>
            ...
            {advancedOpen ? (
              <div className="space-y-1.5">
                <Label htmlFor="edit-profile-key">{l10n("local.identifier_9b10587f")}</Label>
  ...
  if (kind === "duplicate") {
    return (
      <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{l10n("local.duplicate_profile_6ef6a470")}</DialogTitle>
            <DialogDescription>{l10n("local.the_copy_starts_unassigned_unless_you_choose_7a781e20")}</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-1.5">
              <Label htmlFor="copy-profile-name">{l10n("local.name_dcd1d522")}</Label>
              <Input id="copy-profile-name" value={copyName} onChange={(e) => setCopyName(e.target.value)} />
              {duplicateCopyName ? <p className="text-xs text-destructive">{l10n("local.another_profile_already_uses_this_name_023eb0b1")}</p> : null}
            </div>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={copyAssignments} onChange={(e) => setCopyAssignments(e.target.checked)} />
              {l10n("local.also_copy_assignments_7bcfff8c")}</label>
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={onClose}>{l10n("local.cancel_19766ed6")}</Button>
...
function RemoveAssignmentDialog({
  ...
  return (
    <Dialog open={Boolean(binding)} onOpenChange={(next) => !next && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{l10n("local.remove_assignment_9d309b99")}</DialogTitle>
          <DialogDescription>
            {binding?.targetType === "company"
              ? l10n("local.removing_the_organization_default_changes_acc_defe9598")
              : l10n("local.remove_this_profile_from_value_6f407327", {v0: (label)})}
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
          <Button variant="ghost" onClick={onClose}>{l10n("local.cancel_19766ed6")}</Button>
          <Button disabled={pending} onClick={onConfirm}>{l10n("local.remove_c3812fc4")}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
function sourceLabel(entry: ToolProfileEntry | null, app: string): string {
  if (!entry) return l10n("local.added_directly_3fa2db40");
  if (entry.selectorType === "application" || entry.selectorType === "connection") return l10n("local.added_by_rule_all_value_e2eb3fad", {v0: (app)});
  if (entry.selectorType === "risk_level" && entry.riskLevel) return l10n("local.added_by_rule_value_tools_0330d67c", {v0: (entry.riskLevel)});
  return l10n("local.added_directly_3fa2db40");
}

function capabilityLabel(tool: ToolCatalogEntry): string {
  if (tool.isDestructive) return l10n("local.destructive_c3e58a73");
  if (tool.isWrite) return l10n("local.write_3f00927a");
  return l10n("local.read_9b9a8d05");
}
...
function newToolsAppLabel(tools: ToolProfileNewToolReviewItem[]): string {
  ...
  if (names.length === 0) return l10n("local.an_app_094f9581");
  if (names.length === 1) return names[0] ?? "An app";
  if (names.length === 2) return l10n("local.value_and_value_598386a7", {v0: (names[0]), v1: (names[1])});
  return l10n("local.value_and_value_more_apps_f0be5e25", {v0: (names[0]), v1: (names.length - 1)});
}
...
function assignmentLabel(
  ...
  if (binding.targetType === "company") return l10n("local.organization_default_804a1f08");
  ...
  if (binding.targetId === companyId) return l10n("local.organization_d764d425");
  return binding.targetId;
}

function assignmentTypeLabel(type: ToolProfileBinding["targetType"]): string {
  if (type === "company") return l10n("local.organization_default_804a1f08");
  if (type === "agent") return l10n("local.agent_11b39c93");
  if (type === "project") return l10n("local.project_98595978");
  if (type === "routine") return l10n("local.routine_0b5baf30");
  return l10n("local.scoped_assignment_9e2aa70b");
}
