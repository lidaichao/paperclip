...
function getRevisionLabel(revision: DocumentRevision) {
  const actor = revision.createdByUserId
    ...
    : revision.createdByAgentId
      ? "agent"
      : "system";
  return l10n("local.rev_value_value_value_7de7a490", {v0: (revision.revisionNumber), v1: (relativeTime(revision.createdAt)), v2: (actor)});
}
      ...
      <DialogContent className="!max-w-(--pct-90) w-full max-h-(--sz-85vh) overflow-hidden flex flex-col">
        <div className="flex items-center justify-between gap-4">
          <DialogHeader className="shrink-0">
            <DialogTitle>
              {l10n("local.diff_6b2cacfd")}{" "}<span className="font-mono text-sm">{documentKey}</span>
            </DialogTitle>
          </DialogHeader>

          <div className="flex items-center gap-4 shrink-0">
            <div className="flex items-center gap-2">
              <Badge variant="outline" className="border-red-500/30 bg-red-500/10 text-(length:--text-nano) uppercase tracking-wider text-red-400">{l10n("local.old_bca97160")}</Badge>
              <Select
            ...
            <div className="flex items-center gap-2">
              <Badge variant="outline" className="border-green-500/30 bg-green-500/10 text-(length:--text-nano) uppercase tracking-wider text-green-400">{l10n("local.new_18fdd549")}</Badge>
              <Select
        ...
        <div className="overflow-auto flex-1 rounded-md border border-border text-xs">
          {!revisions ? (
            <div className="p-6 text-center text-muted-foreground text-sm">{l10n("local.loading_revisions_c3cd7185")}</div>
          ) : !leftRevision || !rightRevision ? (
            <div className="p-6 text-center text-muted-foreground text-sm">{l10n("local.select_two_revisions_to_compare_e64fab56")}</div>
          ) : leftRevision.id === rightRevision.id ? (
            <div className="p-6 text-center text-muted-foreground text-sm">{l10n("local.both_sides_are_the_same_revision_5c3e0d37")}</div>
          ) : (
