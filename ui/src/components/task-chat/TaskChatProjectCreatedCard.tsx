...
export function TaskChatProjectCreatedCard({ item }: { item: TaskChatProjectCreatedItem }) {
  return (
    <article className="rounded-lg border border-border bg-card p-3" aria-label={l10n("local.project_created_value_99882ac7", {v0: (item.name)})}>
      <div className="flex items-start gap-3">
        <FolderKanban className="mt-0.5 size-4 shrink-0 text-muted-foreground" aria-hidden />
        <div className="min-w-0 flex-1 space-y-1">
          <p className="text-xs text-muted-foreground">{l10n("local.project_created_5f4743ef")}</p>
