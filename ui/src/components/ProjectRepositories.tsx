...
export function ProjectRepositories({ project }: { project: Project }) {
  ...
  return <section aria-label={l10n("local.repositories_1e32af87")} className="flex min-w-0 flex-col gap-4 py-4">
    ...
    <div className="flex flex-wrap items-center justify-end gap-2 border-t border-border pt-4">
      ...
      {save.isSuccess && <span role="status" className="mr-auto text-sm text-muted-foreground">{l10n("local.changes_saved_16bf144a")}</span>}
      {draft && <Button type="button" variant="ghost" disabled={save.isPending} onClick={() => { setDraft(null); save.reset(); }}>{l10n("local.discard_changes_f9bfa3dc")}</Button>}
      <Button type="button" disabled={!draft || save.isPending} onClick={() => save.mutate()}>{save.isPending ? l10n("local.saving_23e39291") : l10n("local.save_changes_dd0ae7a5")}</Button>
    </div>
