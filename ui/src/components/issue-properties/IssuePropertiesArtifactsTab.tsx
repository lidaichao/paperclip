...
function workProductStatusBadge(status: string): { label: string; cssVar: string } | null {
  switch (status) {
    case "active":
    case "draft":
      return { label: l10n("local.in_progress_c1f88e9d"), cssVar: "--status-task-in_progress" };
    case "ready_for_review":
      return { label: l10n("local.for_review_1afa6050"), cssVar: "--status-task-in_review" };
    case "approved":
    case "merged":
      return { label: l10n("local.done_11a6767d"), cssVar: "--status-task-done" };
    case "changes_requested":
      return { label: l10n("local.changes_requested_10a92a8a"), cssVar: "--status-task-todo" };
    case "failed":
      return { label: l10n("local.failed_031a8f0f"), cssVar: "--status-task-blocked" };
    default:
      return null;
  }
}
  ...
  if (tooLarge) {
    expandedBody = (
      <p className="text-sm text-muted-foreground">
        {l10n("local.this_markdown_file_is_too_large_to_preview_us_c4195ee5")}</p>
    );
  } else if (reviewDoc) {
    ...
    expandedBody = (
      <div className="flex flex-col items-start gap-1.5">
        <p className="text-sm text-muted-foreground">
          {unsupportedError
            ? l10n("local.this_file_can_t_be_previewed_as_markdown_use_e2d6081f")
            : l10n("local.preview_failed_to_load_83edf741")}
        </p>
        {!unsupportedError ? (
          <button
            type="button"
    ...
    expandedBody = <p className="text-sm text-muted-foreground">{l10n("local.preparing_preview_11e66e1b")}</p>;
  }
    ...
    <div className="rounded-md border border-border bg-card/50">
      <div ref={headerRef} className="flex items-center hover:bg-accent/50">
        ...
        >
          ...
          {reviewDoc ? (
            <span className="shrink-0 text-(length:--text-micro) text-muted-foreground">
              {l10n("local.rev_value_46689171", {v0: (reviewDoc.latestRevisionNumber ?? 1)})}
            </span>
          ) : null}
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          aria-label={l10n("local.open_raw_value_44f212db", {v0: (workProduct.title)})}
          title={l10n("local.open_raw_7f57bf8c")}
        ...
        <a
          ...
          aria-label={l10n("local.download_value_ac3a0cac", {v0: (workProduct.title)})}
  ...
  if (onOpen) {
    return (
      ...
      >
        ...
        <span className="shrink-0 text-(length:--text-micro) text-muted-foreground">
          {l10n("local.rev_value_46689171", {v0: (doc.latestRevisionNumber ?? 1)})}
        </span>
  ...
  if (workProductRows.length === 0 && documentRows.length === 0 && fileRows.length === 0) {
    return (
      <div className="px-1 py-6 text-sm text-muted-foreground">
        {l10n("local.no_artifacts_yet_work_products_documents_and_b032412c")}</div>
    );
  }
            ...
            {group.runId !== "other" ? (
              <header className="flex items-baseline justify-between gap-2 px-1">
                <h3 className="truncate text-xs font-medium text-foreground">
                  {agent?.name ?? l10n("local.run_value_b0b0fae1", {v0: (group.runId.slice(0, 8))})}
                </h3>
