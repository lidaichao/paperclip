...
function SkillList({ values }: { values: unknown }) {
  ...
  return (
    <div className="flex items-start gap-2">
      <span className="text-muted-foreground w-20 sm:w-24 shrink-0 text-xs pt-0.5">{l10n("local.skills_66d0f523")}</span>
...
export function HireAgentPayload({ payload }: { payload: Record<string, unknown> }) {
  return (
    <div className="mt-3 space-y-1.5 text-sm">
      <div className="flex items-center gap-2">
        <span className="text-muted-foreground w-20 sm:w-24 shrink-0 text-xs">{l10n("local.name_dcd1d522")}</span>
        <span className="font-medium">{String(payload.name ?? "—")}</span>
      </div>
      <PayloadField label={l10n("local.role_14736a2e")} value={payload.role} />
      <PayloadField label={l10n("local.title_7e8cd205")} value={payload.title} />
      <PayloadField label={l10n("local.icon_a35abcd6")} value={payload.icon} />
      {!!payload.capabilities && (
        <div className="flex items-start gap-2">
          <span className="text-muted-foreground w-20 sm:w-24 shrink-0 text-xs pt-0.5">{l10n("local.capabilities_9460f16a")}</span>
          <span className="text-muted-foreground">{String(payload.capabilities)}</span>
        </div>
      )}
      {!!payload.adapterType && (
        <div className="flex items-center gap-2">
          <span className="text-muted-foreground w-20 sm:w-24 shrink-0 text-xs">{l10n("local.adapter_0252b849")}</span>
...
export function BudgetOverridePayload({ payload }: { payload: Record<string, unknown> }) {
  ...
  return (
    <div className="mt-3 space-y-1.5 text-sm">
      <PayloadField label={l10n("local.scope_b073f6c6")} value={payload.scopeName ?? payload.scopeType} />
      <PayloadField label={l10n("local.window_19734a1b")} value={payload.windowKind} />
      <PayloadField label={l10n("local.metric_2d275a74")} value={payload.metric} />
      {(budgetAmount !== null || observedAmount !== null) ? (
        <div className="rounded-md bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
          {l10n("local.limit_674b0ed5")}{" "}{budgetAmount !== null ? formatCents(budgetAmount) : "—"} {l10n("local._observed_552fc380")}{" "}{observedAmount !== null ? formatCents(observedAmount) : "—"}
        </div>
      ) : null}
      {!!payload.guidance && (
    ...
    <div className="mt-4 space-y-3.5 text-sm">
      {title && (
        <div className="space-y-1">
          <p className="text-(length:--text-micro) font-medium uppercase tracking-(--tracking-label) text-muted-foreground">{l10n("local.title_7e8cd205")}</p>
          <p className="font-medium leading-6 text-foreground">{title}</p>
        </div>
      )}
      {summary && (
        <div className="space-y-1">
          <p className="text-(length:--text-micro) font-medium uppercase tracking-(--tracking-label) text-muted-foreground">{l10n("local.summary_8e76a94a")}</p>
      ...
      {recommendedAction && (
        <div className="rounded-lg border border-amber-500/20 bg-amber-500/10 px-3.5 py-3">
          <p className="text-(length:--text-micro) font-medium uppercase tracking-(--tracking-label) text-amber-700 dark:text-amber-300">
            {l10n("local.recommended_action_0cdf19ee")}</p>
          <MarkdownBody className="mt-1 leading-6 text-foreground">{recommendedAction}</MarkdownBody>
        </div>
      )}
      {nextActionOnApproval && (
        <div className="rounded-lg border border-border/60 bg-background/60 px-3.5 py-3">
          <p className="text-(length:--text-micro) font-medium uppercase tracking-(--tracking-label) text-muted-foreground">{l10n("local.on_approval_e65731c1")}</p>
          <MarkdownBody className="mt-1 leading-6 text-foreground">{nextActionOnApproval}</MarkdownBody>
        </div>
      )}
      {risks.length > 0 && (
        <div className="space-y-1.5">
          <p className="text-(length:--text-micro) font-medium uppercase tracking-(--tracking-label) text-muted-foreground">{l10n("local.risks_bb685708")}</p>
      ...
      {proposedComment && (
        <div className="space-y-1.5">
          <p className="text-(length:--text-micro) font-medium uppercase tracking-(--tracking-label) text-muted-foreground">
            {l10n("local.proposed_comment_36e856c1")}</p>
